 ...
 *                0x04 → TVOC-eCO2 (sgp30)
 *                0x05 → AC voltage (mV)
 *                0x06 → power consumption (mW)
 * Returns sensor value as int32_t,
 *         -9999 if unknown address,
 *         -9990 if sensor error
 */
int32_t read_sensor(uint8_t input_address) {
	...
	case 0x03:
		...
		return ((int32_t) temp16 << 16) | hum16;

	case 0x04:
		measure_and_print_iaq();

		if (iaq.eco2_ppm == -9990 || iaq.tvoc_ppb == -9990) {
			return -9990;
		}
		...
		return ((int32_t) eco2 << 16) | tvoc;

	case 0x05:   // AC Voltage (mV)
	{
		int32_t voltage_mv = read_ac_voltage_mv_median();
		if (voltage_mv == -9990)
		    return -9990;
		return voltage_mv;
	}

	case 0x06:   // Power Consumption (mW)
	{
		int32_t power_mw = read_power_mw_median();
		return power_mw;
	}
